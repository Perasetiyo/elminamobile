...
class Material_Inventory extends CI_Controller {
	...
	function push_button() {
		if ($_POST['action'] == 'Cetak Kode') {
			$this->cetak_kode();
		} else if ($_POST['action'] == 'Bulk Emptying') {
			$this->bulk_empty();
		}
	}
	...
	public function lists() {
		if($this->session->userdata('logged_in')) {	
			...
			// Paging
			$config = array();
			$config["base_url"] = base_url("index.php/material_inventory/lists");
	...
	public function bulk_empty() {
		...
		for ($i = 0;$i<= $totalrow;$i++) {
			...
			if ( isset($_POST['ch'.$i]) ) {
				$count++;
				// query material
				$this->db->where('material_id', $ch);
				$this->db->limit(1);
				$material = $this->db->get('tb_material_inventory');
				$material_cogs = $material->row()->material_cogs;
				$material_qty = $material->row()->material_qty;
				$material_nominal = $material->row()->material_nominal;
				$material_code = $material->row()->material_code;
			
				// new material inventory qty & nominal
				$material_last_qty = 0;
				$material_used_nominal = $material_qty * $material_nominal;
				$material_last_nominal = 0;
			
				$cutting_material = array(
								'material_inventory_log_date' => date('Y-m-d', strtotime('now')),
								'material_inventory_id' => $ch,
								'material_inventory_log_desc' => 'Bulk Emptying, cutting '.$material_code,
								'material_inventory_log_used_qty' => $material_qty,
								'material_inventory_log_last_qty' => $material_last_qty,
								'material_inventory_log_used_nominal' => $material_used_nominal,
								'material_inventory_log_last_nominal' => $material_last_nominal
				);
				...
				$this->update_material_balance($ch, $material_last_qty, $material_last_nominal);
			}
		}
		...
		// add to activity
		$session_data = $this->session->userdata('logged_in');
		$msg = '<p>'.$count.' Material was emptied. Material Inv Log has been added..!</p>';
				
		$activity_desc = 'MATERIAL INV - Emptying material [ <b>'.$count.' material(s) </b>]';
		$this->activity_model->add_activity($session_data['user_id'], $activity_desc);
