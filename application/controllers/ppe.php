...
class PPE extends CI_Controller {
	...
	public function doAdd() {
		if($this->session->userdata('logged_in')) {
			...
			$ppe = array(
				   ...
				   'ppe_nominal_cash' => $this->input->post('ppe_nominal_cash'),
				   'bank_account_id' => $this->input->post('bank_account_id'),
				   'ppe_nominal_credit' => $this->input->post('ppe_nominal_credit'),
	...
	public function doJual() {
		if($this->session->userdata('logged_in')) {
			$msg = "";
			$sold_nominal = $this->input->post('sold_nominal_cash') + $this->input->post('sold_nominal_credit');
			
			$this->db->where('ppe_id', $this->input->post('ppe_id')); 
			$query=$this->db->get('tb_ppe');
			$ppe_nominal = $query->row()->ppe_nominal - $query->row()->ppe_has_been_paid;
			
			if ($ppe_nominal > $sold_nominal) { // RUGI
				// insert into expense
				$expense = array(
					   'expense_type_id' => $this->input->post('ppe_type_id'),
					   'expense_date' => date('Y-m-d', strtotime($this->input->post('sold_date'))),
					   'expense_desc' => 'Penjualan aset lebih murah dari Net PPE : '.$this->input->post('ppe_desc'),
					   'expense_nominal' => $ppe_nominal - $sold_nominal
					);
				$this->db->insert('tb_expense', $expense); 
				$msg .= '<p>Expense ('.$this->input->post('expense_desc').') has been added..!</p>';

			} else if ($ppe_nominal < $sold_nominal) { // UNTUNG
				// insert into equity
				$equity = array(
					   'equity_date' => date('Y-m-d', strtotime($this->input->post('sold_date'))),
					   'equity_desc' => 'Keuntungan penjualan aset : '.$this->input->post('ppe_desc'),
					   'equity_nominal' => $sold_nominal - $ppe_nominal,
					   'equity_type_id' => 19, // additional investment
					   'bank_account_id' => $this->input->post('bank_account_id')
					);
				$this->db->insert('tb_equity', $equity); 
				$msg .= '<p>Equity ('.$this->input->post('expense_desc').') has been added..!</p>';
			...
			if ($this->input->post('sold_nominal_cash') > 0) {
				// adding cash
				$cash = array (
						'cash_type_id' => $this->input->post('ppe_type_id'),
						'cash_date' => date('Y-m-d', strtotime($this->input->post('sold_date'))),
						'cash_desc' => 'Penjualan aset : '.$this->input->post('ppe_desc'),
						'cash_nominal' => $this->input->post('sold_nominal_cash'),
						'bank_account_id' => $this->input->post('bank_account_id'),
						'ppe_id' => $this->input->post('ppe_id')
					);
				...
				$msg .= '<p>Cash has been increased '. $this->input->post('sold_nominal_cash') .' rupiahs!</p>';
			}
			
			if ($this->input->post('sold_nominal_credit') > 0) {
				// add accrec / piutang
				$acrec = array (
						'acrec_type_id' => $this->input->post('ppe_type_id'),
						'acrec_date' => date('Y-m-d', strtotime($this->input->post('sold_date'))),
						'acrec_desc' => 'Penjualan aset '.$this->input->post('ppe_desc'),
						'acrec_nominal' => $this->input->post('sold_nominal_credit'),
						'ppe_id' => $this->input->post('ppe_id')
					);
				
				$this->db->insert('tb_acrec', $acrec); 
				
				$msg .= '<p>Accrec has been increased '. $this->input->post('ppe_nominal_credit') .' rupiahs!</p>';
			}
			$this->session->set_flashdata('success_message',$msg);	
			redirect(site_url('ppe/lists'));
		} else {
	...
	public function jual() {
		if($this->session->userdata('logged_in')) {
			...
			$this->db->from('tb_ppe');
			$this->db->join('tb_options', 'tb_options.option_id = tb_ppe.ppe_type_id','left');
			$this->db->where('tb_ppe.ppe_id',$this->uri->segment(3));
